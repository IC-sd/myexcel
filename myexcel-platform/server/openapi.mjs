...
export const openApiV1 = {
  ...
  paths: {
    '/': { get: { tags: ['Contract'], security: [], summary: 'Read API versions', responses: { 200: json({ type: 'object' }) } } },
    '/openapi.json': { get: { tags: ['Contract'], security: [], summary: 'Read this OpenAPI document', responses: { 200: json({ type: 'object' }) } } },
    ...
    '/template-packages/validate': { post: { tags: ['Template packages'], summary: 'Validate a package without writing it', requestBody: { required: true, content: { 'application/json': { schema: ref('TemplatePackage') } } }, responses: { 200: json({ type: 'object', required: ['report'], properties: { report: ref('PackageValidation') } }), 422: json(ref('ValidationError')) } } },
